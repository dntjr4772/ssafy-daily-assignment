 ...
 * 블럭이동 (배열행 이동)
 */
public class Main_백준_17135_캐슬디펜스_한우석 {
	...
	public static void main(String[] args) throws IOException {
		...
		D = Integer.parseInt(st.nextToken()); // 궁수의 공격 거리 제한 D, 1 ≤ D ≤ 10
	...
	public static int shot() {
		...
		int total = sum; // 맵에 남아있는 적군의 수
		for (int i = 0; total > 0 && i < N; i++) { // N행의 수만큼 && 적군이 있을때까지
			hs.clear(); // 초기화
...
ex:				for (int k = 1; k <= D; k++) { // 공격거리
			...
			cnt += hs.size();
			total -= hs.size(); // 잡은 적군 수만큼 맵에서 제거
			for (Integer x : hs) {
				map[x / M][x % M] = 0;
			}
			total -= move();
	...
	/** 맵을 한줄 씩 내리기, 맵밖으로 떨어진 적군의 수를 리턴 */
